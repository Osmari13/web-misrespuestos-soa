...
from controller import cliente, conexion, inventario, registro_venta, reportes
from flask import Flask, flash, redirect, render_template, request

app = Flask(__name__, template_folder="template")

app.config.from_mapping(SECRET_KEY="llavesecreta")  # para evitar vulnerabilidades
...
def add_inventario():
    ...
    try:
        inventario.insertar_inventario(
            nombre,
            precio,
            serie,
            pago_adquisicion,
...
@app.route(
    "/form_editar_inventario/<id>"
)  # devuelve el producto del inventario que se quiere editar dependiendo del id
...
def ingresar_registroventa():
    try:
        total_precio = 0.0
        id_inventario = request.form.getlist("id_inventario[]")
        cantidad_venta = request.form.getlist("cantidad_venta[]")

        id_cliente = request.form["id_cliente"]

        if id_inventario != "" and id_cliente != "":
            ventas = [(cant_venta) for cant_venta in cantidad_venta if cant_venta]
            
            for id_ivn, cant_venta in zip(
                id_inventario, ventas
            ):  # Iterate over both lists together
                inv_ventas = inventario.listar_inventario_id(id_ivn)
                clint_ventas = cliente.listar_cliente_id(id_cliente)

                if isinstance(inv_ventas, tuple) and isinstance(clint_ventas, tuple):
                    if cant_venta:
                        if int(cant_venta) <= inv_ventas[0][4]:
                            total_precio = int(cant_venta) * inv_ventas[0][2]

                            registro_venta.insertar_registroventa(
                                id_cliente, id_ivn, cant_venta, total_precio
                            )
                        else:
                            flash("Cantidad no disponible", "error")
        ...
        flash("Un error ha ocurrido: {}".format(str(e)), "error")
        return redirect("/tabla_registroventa")
...
def tabla_registroventa():
    ...
    return render_template(
        "registro_venta/registro_registroventa.html",
        ventas=ventas,
        cliente_ventas=cliente_ventas,
        inventario_ventas=inventario_ventas,
    )


@app.route(
    "/form_editar_registroventa/<id>"
)  # lo que hace es devolver el registro que se quiere editar dependientdo del ID
def editar_venta(id):
    ...
    conex.close()
    return render_template(
        "registro_venta/editar_venta.html",
        venta=venta[0],
    ...
    if id_inventario != "" and id_cliente != "":
        ...
        if type(inv_venta) == tuple and type(clint_venta) == tuple:
            ...
            if int(cantidad_venta) <= int(inv_venta[0][4]):
                ...
                registro_venta.actualizar_registroventa(
                    id_cliente,
                    id_inventario,
                    cantidad_venta,
                    total_precio,
...
def reporte_inventario_sem():
    try:
        ...
        print(e)
    return render_template(
        "reportes/semanales/reportes-inventario-sem.html",
        rep_inventario_sem=rep_inventario_sem,
    )
...
def reporte_inventario_mes():
    try:
        ...
        print(e)
    return render_template(
        "reportes/mensuales/reportes-inventario-mes.html",
        rep_inventario_mes=rep_inventario_mes,
    )
...
def reporte_deficit_sem():
    try:
        ...
        print(e)
    return render_template(
        "reportes/semanales/reportes-deficit-sem.html",
        rep_defProductSem=rep_defProductSem,
    )
...
def reporte_deficit_mes():
    try:
        ...
        print(e)
    return render_template(
        "reportes/mensuales/reportes-deficit-mes.html",
        rep_defProductMes=rep_defProductMes,
    )
...
def reporte_stado_sem():
    try:
        ...
        print(e)
    return render_template(
        "reportes/semanales/reportes-stado-sem.html", rep_stCuentaSem=rep_stCuentaSem
    )
...
def reporte_stado_mes():
    ...
    return render_template(
        "reportes/mensuales/reportes-stado-mes.html", rep_stCuentaMes=rep_stCuentaMes
    )
...
def reporte_ventas_sem():
    try:
        ...
        print(e)
    return render_template(
        "reportes/semanales/reportes-ventas-sem.html", rep_ventasSem=rep_ventasSem
    )
...
def reporte_ventas_mes():
    try:
        ...
        print(e)
    return render_template(
        "reportes/mensuales/reportes-ventas-mes.html", rep_ventasMes=rep_ventasMes
    )
